    ...
    async execute(interaction, client) {
        ...
        if (sub === 'dar') {
            ...
            if (data.bans.find(b => b.id === jugador.id)) {
                return interaction.reply({ content: `⚠️ **${jugador.username}** ya está baneado.`, ephemeral: true });
            }

            // Quitar de todos los equipos
            ...
            // Quitar roles de la liga en Discord
            const member = await interaction.guild.members.fetch(jugador.id).catch(() => null);
            if (member) {
                await member.roles.remove(client.config.roles.agentesLibres).catch(() => {});
                await member.roles.remove(client.config.roles.dt).catch(() => {});
            }

            const embed = new EmbedBuilder()
                .setTitle('🔨 Jugador Baneado')
                .setDescription(`**${jugador.username}** ha sido baneado de la liga.\n**Motivo:** ${motivo}`)
                .setColor(0xFF0000)
                .setTimestamp();
            ...
            if (index === -1) {
                return interaction.reply({ content: `⚠️ **${jugador.username}** no está baneado.`, ephemeral: true });
            }
            data.bans.splice(index, 1);
            saveData(data);
            await interaction.reply({ content: `✅ **${jugador.username}** desbaneado de la liga.` });
